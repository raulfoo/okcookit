...
namespace :fezzik do
  ...
  remote_task :pre_push_setup, :roles => :root_user do
    ...
    add_user("okcookit") unless has_user?("okcookit")
    run "ruby --version > /dev/null || apt-get install -y ruby"
    run "rsync --version > /dev/null || apt-get install -y rsync"
    run "mkdir -p /opt/#{app}/releases && chown -R okcookit /opt/#{app}"
  end

  desc "performs post-push setup"
  remote_task :post_push_setup, :roles => :okcookit_user do
  ...
  remote_task :generate_upstart_scripts, :roles => :okcookit_user do
